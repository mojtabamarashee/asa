...
var file = fs.createWriteStream(
  outPath + 'out_' + date + '/Nav1D_' + date + '.html',
);
...
var file = fs.createWriteStream(
  outPath + 'out_' + date + '/tagh1D_' + date + '.html',
);
file.write(htmlHeader);
file.write(
  '<thead><tr>' +
...
var file = fs.createWriteStream(
  outPath + 'out_' + date + '/rsi_' + date + '.html',
);
file.write(htmlHeader);
file.write(
  '<thead><tr>' +
...
var file = fs.createWriteStream(
  outPath + 'out_' + date + '/sell_' + date + '.html',
);
file.write(htmlHeader);
file.write(
  '<thead><tr><th>نماد</th>' +
...
var file = fs.createWriteStream(
  outPath + 'out_' + date + '/buy_' + date + '.html',
);
file.write(htmlHeader);
file.write(
  '<thead><tr>' +
...
var file = fs.createWriteStream(
  outPath + 'out_' + date + '/1wBaz_' + date + '.html',
);
file.write(htmlHeader);
file.write(
  '<thead><tr>' +
      ...
      if (v.hist.length > 30 && v.hist[6].PDrCotVal > 0) {
        ...
        file.write(
          '<tr><td>' +
            ...
            '</td><td>' +
            numeral(
              ((v.pl - v.hist[6].PDrCotVal) / v.hist[6].PDrCotVal) * 100,
            ).format() +
            '</td><td>' +
            v.flow +
            '</td><td style="color:#' +
...
var file = fs.createWriteStream(
  outPath + 'out_' + date + '/ct_' + date + '.html',
);
file.write(htmlHeader);
file.write(
  '<thead><tr><th>نماد</th>' +
...
var file = fs.createWriteStream(
  outPath + 'out_' + date + '/floatVal_' + date + '.html',
);
file.write(htmlHeader);
file.write(
  '<thead><tr>' +
...
var file = fs.createWriteStream(
  outPath + 'out_' + date + '/pe_' + date + '.html',
);
file.write(htmlHeader);
file.write(
  '<thead><tr>' +
...
var file = fs.createWriteStream(
  outPath + 'out_' + date + '/min60D_' + date + '.html',
);
file.write(htmlHeader);
file.write(
  '<thead><tr>' +
...
var file = fs.createWriteStream(
  outPath + 'out_' + date + '/filter_' + date + '.txt',
);
allRows
  ...
  .filter(
    (v, i) => v.pc < 0.8 * Math.max.apply(null, v.hist.map(v => v.PClosing)),
  )
  .forEach((v, i) => {
    file.write(v.l18 + '\n');
  });
...
allRows.forEach((v, i) => {
  ...
  if (v.hist) {
    for (i1 = 0; i1 < v.hist.length - 1; i1++) {
      if (
        Math.abs(v.hist[i1].PClosing - v.hist[i1 + 1].PClosing) /
          v.hist[i1 + 1].PClosing >
        0.2
      ) {
...
var file = fs.createWriteStream(
  outPath + 'out_' + date + '/rsi_' + date + '.txt',
);
allRows
...
var file = fs.createWriteStream(
  outPath + 'out_' + date + '/pe_' + date + '.txt',
);
t = allRows
  .filter((v, i) => v.cs == 44)
...
var file = fs.createWriteStream(
  outPath + 'out_' + date + '/ser_' + date + '.txt',
);
//t = allRows
      ...
      if (v.l18.match(/^([^0-9]*)$/) && !v.pClosingHist) {
        ...
        axios
          ...
          .then(response => {
            ...
            if (i == 1) console.log('data = ', response.data);
            if (v.l18 == 'فسا')
              fs.appendFile('error.txt', response.data + '\n', () => {});
...
allRows.forEach((v, i) => {
  if (v.pClosingHist) {
    ...
    if (v.hist[n])
      v.d5 = numeral(
        -((v.pClosingHist[n] - v.pc) / v.pClosingHist[n]) * 100,
      ).format();
...
setTimeout(() => {
  fs.writeFileSync(
    '../onePage/public/allRows_' + date + '.js',
    'allRows=' + JSON.stringify(allRows),
  );
  fs.writeFileSync(
    '../smojmar.github.io/allRows_' + date + '.js',
    'allRows=' + JSON.stringify(allRows),
  );
